...
class TopologyHealth:
    ...
    def analyze(self) -> dict:
        ...
        in_vals = list(in_vals_map.values())
        out_vals = list(out_vals_map.values())
        reachable = False
        shortest = None
            ...
            while q:
                ...
                for syn in n.synapses.get(node, []):
                    if syn.target_id not in visited:
                        ...
                        q.append((syn.target_id, depth + 1))
        return {
            "neurons": len(neurons),
            ...
            "isolated": sum(
                in_vals_map[nid] == 0 and out_vals_map[nid] == 0 for nid in neurons
            ),
