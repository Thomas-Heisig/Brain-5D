...
def cognition_catalog(research_root: Path) -> list[dict[str, Any]]:
    ...
    for value in cast(list[object], values):
        ...
        if item.get("consciousness_inference") != "not_established":
            raise CognitionGovernanceError("A task catalogue cannot certify consciousness")
...
def guard_cognition_launch(research_root: Path, question_id: str, protocol: str) -> None:
    ...
    if state_path.exists():
        try:
            ...
            raise CognitionGovernanceError("Ethics state unreadable; new research launches blocked") from exc
        if not isinstance(state, dict) or state.get("state") not in {"NORMAL", "REVIEW_REQUIRED", "HOLD"}:
...
def assess_candidate(candidate: dict[str, object]) -> dict[str, object]:
    ...
    required = {"candidate_id", "protocol_id", "question_id", "hypothesis_id",
                "source_commit", "raw_artifacts", "analysis_hash", "claim_scope",
                "limitations", "alternatives", "replication", "ethics_review",
                "theory_assumptions", "measurement_validity", "human_review"}
    ...
    return {"status": "INCOMPLETE" if reasons else "READY_FOR_EXTERNAL_REVIEW",
            "reasons": reasons, "authority": "candidate_only",
            "accepted_evidence": False, "reviewer_identity_authenticated": False,
