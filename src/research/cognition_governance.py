...
def cognition_catalog(research_root: Path) -> list[dict[str, Any]]:
    ...
    for value in cast(list[object], values):
        ...
        ids.add(identifier)
        if item.get("consciousness_inference") != "not_established" or item.get("native_adapter_validated") is not False:
            raise CognitionGovernanceError("Catalogue cannot certify consciousness or an unimplemented adapter")
        result.append(dict(item))
    return result


def guard_cognition_launch(research_root: Path, question_id: str, protocol: str) -> None:
    ...
    if state_path.exists():
        ...
        if state not in {"NORMAL", "REVIEW_REQUIRED", "HOLD"}:
            raise CognitionGovernanceError("Invalid ethics state; new research launches blocked")
        if state != "NORMAL":
            raise CognitionGovernanceError("ETHICS_HOLD: new experiment launches blocked; stop/isolate remains available")
    elif (research_root / PROGRAM).exists():
        raise CognitionGovernanceError("Ethics state missing for installed programme; new launches blocked")
    if protected:
...
def guard_cognition_promotion(hypothesis_id: str, claim_id: str) -> None:
    ...
    if hypothesis_id.startswith(HYPOTHESIS_PREFIXES) or claim_id.startswith(("CLAIM-CNS-", "CLAIM-WEL-", "CLAIM-EPI-1")):
        raise CognitionGovernanceError(
...
def _texts(value: object) -> bool:
    return isinstance(value, list) and bool(value) and all(_nonempty_text(item) for item in cast(list[object], value))
...
def assess_candidate(candidate: dict[str, object]) -> dict[str, object]:
    ...
    if isinstance(question, str) and isinstance(hypothesis, str) and isinstance(protocol, str):
        ...
        if not hypothesis.startswith("H-" + key + "-") or not protocol.startswith("cog_" + key.lower().replace("-", "_") + "_v"):
    ...
    if not isinstance(raw, list) or not raw:
        ...
        for index, item in enumerate(cast(list[object], raw)):
            ...
            if not _nonempty_text(path_value) or not _matches(record.get("sha256"), r"[0-9a-f]{64}") or role not in {"primary_observation", "stimulus_schedule", "state", "configuration"}:
                reasons.append(f"invalid:raw_artifacts:{index}")
