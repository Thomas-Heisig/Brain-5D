...
def run_ping(config: Config, seeds: tuple[int, ...] = (42, 43, 44)) -> list[ScientificRun]:
    ...
    for seed in seeds:
        for recurrence in (False, True):
            ...
            signature: NetworkResponseSignature = NetworkImpulseProbe(
                source_neuron=source, current=100.0, max_ticks=8, state_digest=lambda: _digest(network)
            ).run(_ProbeRuntime(network))
            runs.append(ScientificRun(
                "EXP-PING-0001", "recurrence_on" if recurrence else "recurrence_off", seed,
                signature.to_dict(), before, _digest(network),
            ))
    return runs
...
def run_time(
    ...
    for seed in seeds:
        for ticks in tick_counts:
            ...
            runs.append(ScientificRun(
                "EXP-TIME-0001", str(ticks), seed,
                {"ticks": ticks, "duration_seconds": duration, "ticks_per_second": ticks / duration},
                before, _digest(network),
            ))
    return runs
...
def run_5d(
    ...
    for seed in seeds:
        for condition, shape in dimensions.items():
            network = _network(config, seed, dimensions=shape, random_graph=condition == "random_graph")
            before = _digest(network)
            signature = NetworkImpulseProbe(
                source_neuron=min(network.input_cells), current=100.0, max_ticks=8,
            ).run(_ProbeRuntime(network))
            runs.append(ScientificRun(
                "EXP-5D-0001", condition, seed,
                {"dimensions": list(shape), **signature.to_dict()}, before, _digest(network),
            ))
    return runs
...
def run_temporal(config: Config, seeds: tuple[int, ...] = (42, 43, 44)) -> list[ScientificRun]:
    ...
    for seed in seeds:
        network = _network(config, seed)
        before = _digest(network)
        memory = TemporalStateMemory(horizons={"fast": 2, "medium": 4, "slow": 6}, capacity=32)
        comparator = TemporalComparator()
        comparisons: list[dict[str, Any]] = []
        for _ in range(8):
            result = network.step()
            frame = TemporalStateFrame.from_mapping(
                network.current_tick, _digest(network),
                {"mean_v": sum(neuron.v for neuron in network.neurons.values()) / len(network.neurons),
                 "spikes_this_tick": float(len(result.spike_ids)),
                 "synapse_count": float(network.synapse_count)},
            )
            for horizon in memory.horizons:
                comparisons.append(comparator.compare(frame, memory.reference(frame.tick, horizon), horizon=horizon).to_dict())
            memory.append(frame)
        runs.append(ScientificRun(
            "EXP-TEMP-0001", "fast_medium_slow", seed,
            {"comparisons": comparisons, "novelty": "not_registered", "prediction_error": "not_available"},
            before, _digest(network),
        ))
    return runs


def run_stdp(config: Config, seeds: tuple[int, ...] = (42, 43, 44)) -> list[ScientificRun]:
    ...
    for seed in seeds:
        ...
        runs.append(ScientificRun(
            "EXP-STDP-0002", "productive_reward_stdp", seed, asdict(result), "", "",
        ))
    return runs


def run_learning_repeat(config: Config, seeds: tuple[int, ...] = (42, 43, 44)) -> list[ScientificRun]:
    ...
    for seed in seeds:
        ...
        result = run_learning_experiment(values)
        runs.append(ScientificRun(
            "EXP-EMB-0001", "learning_before_after", seed,
            {"success_before": result.baseline_target_spiked, "success_after": result.trained_target_spiked,
             "p_success_before": float(result.baseline_target_spiked),
             "p_success_after": float(result.trained_target_spiked),
             "after_greater_than_before": result.trained_target_spiked and not result.baseline_target_spiked},
            "", "",
        ))
    return runs
...
def write_data(path: Path, runs: list[ScientificRun]) -> None:
    ...
    path.write_text(json.dumps([asdict(run) for run in runs], indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    parser.add_argument("--output", default="research/generated/data/science_suite.json")
