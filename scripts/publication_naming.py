...
def verify(root: Path) -> dict[str, int]:
    ...
    manifest: dict[str, Any] = json.loads((folder / "manifest.json").read_text(encoding="utf-8"))
    ...
    if manifest["title_en"] != identity["publication"]["title_en"] or manifest["subtitle_de"] != identity["publication"]["subtitle_de"]:
        raise ValueError("Publication title differs from canonical identity")
    if manifest["new_empirical_findings"] is not False or manifest["automatic_evidence_promotion"] is not False:
    ...
    for name in order[:-1]:
        anchors = re.findall(r'<a id="([^"]+)"', (old / name).read_text(encoding="utf-8"))
    ...
    audit: dict[str, Any] = json.loads((root / "docs/05-quality/mhrn-naming-audit.json").read_text(encoding="utf-8"))
    ...
    pages = list(folder.glob("*.md")) + [root / "research/publications/README.md", root / "NAMING.md"]
    links = check_links(root, pages)
    return {"sections": 57, "links": links, "historical_files": len(audit["preserved_sha256"])}
...
def export_markdown(root: Path, revision: str) -> str:
    ...
    folder = root / "research/publications" / EDITION
    identity: dict[str, Any] = json.loads((root / "project_identity.json").read_text(encoding="utf-8"))
    platform: dict[str, Any] = json.loads((root / "docs/05-quality/mhrn-platform-migration.json").read_text(encoding="utf-8"))
    repo = platform["github"]["actual"]
    order = [f"section-{index:03d}.md" for index in range(57)]
    title = identity["publication"]["title_en"]
    output = [f'<a id="edition-index"></a>\n# {title}\n\nDerived export from commit `{revision}`; do not edit as a second master.\n']
    for name in order:
        text = "\n".join(line for line in (folder / name).read_text(encoding="utf-8").splitlines() if not line.startswith("[Inhalts"))
...
def main() -> None:
    ...
    if args.export:
        ...
        dirty = subprocess.check_output(["git", "status", "--porcelain", "--", "project_identity.json", "research/publications", "docs/05-quality/mhrn-platform-migration.json"], cwd=root, text=True)
        if dirty.strip():
            raise ValueError("Commit publication sources before exporting a source-bound edition")
