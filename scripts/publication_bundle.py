...
def split_sections(text: str) -> list[tuple[str, int, int, str]]:
    ...
    for index, line in enumerate(lines):
        ...
        if start and ANCHOR.fullmatch(lines[start - 1].strip()):
            start -= 1
        if start > starts[-1] and any(line.strip() for line in lines[starts[-1]:start]):
            starts.append(start)
    result = []
...
def reader_files(source: bytes) -> dict[str, bytes]:
    ...
    for index, (name, (title, first, last, content)) in enumerate(zip(names, sections)):
        ...
        nav = " | ".join(navigation)
        rendered = (nav + "\n\n" + LINK.sub(rewrite, content).rstrip() + "\n\n" + nav + "\n").encode()
        if len(rendered) >= MAX_SECTION_BYTES:
            raise ValueError(f"Reader section is too large for a complete preview: {title}")
        outputs[name] = rendered
        entries.append({
            "path": name, "title": title, "first_line": first, "last_line": last,
            "source_sha256": digest(content.encode()), "rendered_sha256": digest(rendered),
    ...
    index_lines += ["", "Generierte Leseansicht; Herkunft und Abschnittspruefsummen: "
                    "[manifest.json](manifest.json).", ""]
    outputs["README.md"] = "\n".join(index_lines).encode()
    manifest = {"schema_version": "1.0", "source": f"../{PACKAGE}/{MASTER}",
                "source_sha256": digest(source), "source_bytes": len(source),
                "authority": "interpretation_only", "section_count": len(entries),
                "sections": entries}
...
def build_reader(root: Path, *, check: bool = False) -> int:
    ...
    if not check:
        ...
        if existing - set(expected):
            raise ValueError("Unexpected reader files; refusing to silently delete content")
...
def verify_bundle(root: Path) -> int:
    ...
    publication_root = root / PUBLICATIONS
    manifest = json.loads((publication_root / "integrity.json").read_text(encoding="utf-8"))
    ...
    for entry in entries:
        ...
        if len(data) != entry["size"] or digest(data) != entry["sha256"]:
            raise ValueError(f"Checksum mismatch or unmaterialized LFS: {entry['path']}")
    archive = manifest["archive"]
    archive_path = safe_path(publication_root, archive["path"])
    archive_bytes = archive_path.read_bytes()
    if len(archive_bytes) != archive["size"] or digest(archive_bytes) != archive["sha256"]:
...
def main() -> None:
    ...
    if args.build_reader:
        ...
        print(f"VERIFIED: {count} frozen files, original ZIP, {sections} complete reader sections")
