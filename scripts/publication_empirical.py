...
def measured_table(plan: dict[str, Any], summary: dict[str, Any], prefix: str) -> str:
    ...
    for spec, item in zip(plan["selections"], summary["protocols"]):
        ...
        lines += [
            f"### {item['protocol']}",
            "",
            f"Ausfuehrung: {item['status']}; {item['runs']} gespeicherte Laeufe.",
            "",
        ...
        for condition, group in item.get("conditions", {}).items():
            ...
            for metric in spec["primary_outcomes"]:
                ...
                if number:
                    values.append(
                        f"{metric}: {number['mean']:.6g} [{number['min']:.6g};{number['max']:.6g}]"
                    )
            ...
            lines.append(
                f"| {condition} | {group['runs']} | {'; '.join(values) or 'Sieben Basisfamilien; siehe vollstaendige Metriken in summary.json'} |"
            )
...
def publish(source: Path, amendment: Path) -> None:
    ...
    if (
        original["source"] != "f9ed3c2153858b14face0b1d8410741c662fb028"
        or original["total_runs"] != 1272
    ):
        raise ValueError(
            "This dated interpretation requires the reviewed original campaign"
        )
    folder = ROOT / "research/publications" / EDITION
    if folder.exists() or any(
        (ROOT / "research/experiments" / name).exists() for name in CAMPAIGNS
    ):
    ...
    chosen = {
        "dimensional_connectivity_v1": "output_spikes",
        "native_association_holdout_v1": "test_accuracy",
        "brian2_single_neuron_v1": "max_abs_voltage_error",
        "active_scaling_v2": "ticks_per_second",
    }
    ...
    save(
        target / "key_results.json",
        {"measurements": key_results, "accepted_evidence": False},
    )
    for directory, data in datasets:
        save(
    ...
    chapter57 = (
        ...
        + "\n"
        + measured_table(
            load(target / "plan.json"), summary, "../../experiments/EXP-EMP-20260910/"
        )
    )
    chapters = {
        57: chapter57,
    ...
    navigation = "\n".join(
        f"- [{next((line[2:] for line in p.read_text(encoding='utf-8').splitlines() if line.startswith('# ')), p.stem)}]({p.name})"
        for p in sections
    )
    (folder / "README.md").write_text(
        header
        + "[Gesamte Lesefassung](MANUSCRIPT.md) - [Manifest](manifest.json)\n\n"
        + navigation
        + "\n",
        encoding="utf-8",
    )
    ...
    manifest.update(
        ...
        date="2026-09-10",
        section_count=61,
        section_order=[p.name for p in sections],
        inherited_edition="../" + PREVIOUS,
        inherited_file_sha256={
            p.name: sha(p) for p in previous.iterdir() if p.is_file()
        },
        revision_scope="exploratory_measurements_and_limitations",
        new_empirical_findings=True,
        accepted_evidence=False,
        automatic_evidence_promotion=False,
        authority="interpretation_only",
        human_review="pending",
        campaigns={
            d.name: {"source": a["source"], "summary_sha256": sha(d / "summary.json")}
            for d, a in datasets
        },
    )
    manifest["files"] = {
    ...
    index.write_text(
        header
        + f"[Aktuelle Fassung 1.4]({EDITION}/README.md) - [Messdaten und Auswertung](../experiments/EXP-EMP-20260910/ANALYSIS.md)\n\n<details><summary>Historischer Index bis 1.3 (damaliger Stand)</summary>\n\n"
...
def verify() -> None:
    ...
    if (
        manifest["section_count"] != 61
        or manifest["accepted_evidence"]
        or manifest["automatic_evidence_promotion"]
    ):
        raise ValueError("Edition scope/authority mismatch")
    for name, item in manifest["files"].items():
        p = (folder / name).resolve()
        if (
            p.parent != folder.resolve()
            or sha(p) != item["sha256"]
            or p.stat().st_size != item["size"]
        ):
    ...
    for name in CAMPAIGNS:
        target = ROOT / "research/experiments" / name
        if (
            sha(target / "summary.json")
            != manifest["campaigns"][name]["summary_sha256"]
        ):
        ...
        for p in target.glob("*/runs.json.gz"):
            ...
            if (
                hashlib.sha256(gzip.decompress(p.read_bytes())).hexdigest()
                != receipt["uncompressed_data_sha256"]
            ):
    ...
    print(
        "Empirical edition:61 chapters, both DATA inventories and historical checksums verified"
    )
