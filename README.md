...
- [Starting components manually](#starting-components-manually)
- [Operator runtime controls](#operator-runtime-controls)
- [Structural approval](#structural-approval)
- [Snapshot behavior](#snapshot-behavior)
- [Testing](#testing)
- [Static quality checks](#static-quality-checks)
- [Recommended pre‑push sequence](#recommended-pre-push-sequence)
- [Type‑safety policy](#type-safety-policy)
- [Windows notes](#windows-notes)
- [Project structure](#project-structure)
- [Research and design documents](#research-and-design-documents)
- [Roadmap](#roadmap)
- [Next evolution — v0.5.0‑alpha.6](#next-evolution--v050-alpha6)
- [Safety and scope](#safety-and-scope)
- [License and contribution status](#license-and-contribution-status)
- [Current milestone summary](#current-milestone-summary)
- [Repository resources](#repository-resources)
- [Start‑to‑dashboard workflow](#start-to-dashboard-workflow)
...
The runtime checkpoint preserves state not fully represented by the frozen snapshot alone, including:
...
The structural replay occurs **before** the runtime checkpoint is overlaid, preserving the expected network topology for deterministic continuation.

---

## Operator dashboard

Brain‑5D includes a local operator dashboard that acts as the system control and inspection surface.
...
The dashboard does **not** directly manipulate `NeuralNetwork` internals. Mutating operations are routed through the operator/controller/bridge boundaries.
...
The development environment includes the quality tooling required by the project, including `pytest`, `Black`, `mypy`, `Pylint` and `Pyright`.
...
If PowerShell reports that the script is not digitally signed, either use the Python launcher above or the CMD wrapper below.
...
The `src.main` CLI intentionally has its own argument contract. Launcher‑specific arguments are not forwarded blindly to it.
...
If no usable snapshot/network source is attached, some heatmap or runtime controls may be unavailable by design.
...
Controller requests are validated. Negative tick counts and values above the configured manual limit are rejected.
...
Auto‑approval is only permitted when all configured confidence, cooldown and safety limits are satisfied.

---

## Snapshot behavior

A dashboard snapshot request does **not** write arbitrary state immediately in the HTTP request thread.
...
This prevents the operator interface from capturing a topology in the middle of a controlled structural mutation batch.
...
The verifier is intended to call tools through the active Python environment instead of silently depending on globally installed executables.
...
Historical complexity/documentation findings are still present in parts of the codebase. They are tracked separately from alpha.5 functional correctness.
...
This prevents false discrepancies between VS Code/Pylance, pytest, mypy and the actual project environment.
...
The project contains research/design material that informs future experiments. These documents are inputs to engineering decisions, not automatically implemented features.
...
A research proposal becomes a project milestone only after it has a concrete experiment, measurable acceptance criteria and a defined software boundary.

---

## Roadmap

The active v0.5 development line is moving from controlled structural mutation toward stable morphological self‑regulation.
...
The detailed v0.5 roadmap is maintained in the organized roadmap documentation under `docs/Roadmap/`.
...
Alpha.6 should not simply increase autonomous mutation. It should make structural plasticity temporally and spatially stable.
...
These belong to later milestones and require separate safety, resource and evaluation contracts.
...
Before substantial external contribution or redistribution, check the repository for the current license and contribution policy.
...
The next engineering target is long‑term **morphological stability**, not unrestricted autonomy.
...
```powershell
.\.venv\Scripts\python.exe scripts\brain5d_launcher.py start --open-browser
```
...
If PowerShell execution policy blocks `.ps1` files, use the Python launcher or the provided `.cmd` wrappers instead of changing the machine‑wide execution policy solely for Brain‑5D.
...
Repository‑wide strict Pyright still contains historical findings outside the alpha.5 scope, so new code must not add additional strict‑type debt.
...
Contributions are welcome when they preserve the typed architecture, safety boundaries, deterministic persistence contracts, and existing tests.
...
For security‑sensitive findings, do **not** publish exploit details in a normal issue. Use GitHub's private security‑reporting facilities when available.